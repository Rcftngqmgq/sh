...
while true; do
	...
	curl -s --max-time 15 ${gh_proxy}raw.githubusercontent.com/Rcftngqmgq/sh/main/kejilion_sh_log.txt | tail -n 30
	# 只下载前5行获取版本号，避免下载整个脚本
	local sh_v_new=$(curl -s --max-time 15 -r 0-200 ${gh_proxy}raw.githubusercontent.com/Rcftngqmgq/sh/main/kejilion.sh | grep -o 'sh_v="[0-9.]*"' | head -1 | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local download_url
			if [ "$country" = "CN" ]; then
				download_url="${gh_proxy}raw.githubusercontent.com/Rcftngqmgq/sh/main/kejilion.sh"
			else
				download_url="${gh_proxy}raw.githubusercontent.com/Rcftngqmgq/sh/main/kejilion.sh"
			fi
